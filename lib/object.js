...
define(function() {//global scope
  ...
  Q.mixin(Q.WARNINGS, {
    ...
    "multiple_ancestors": "Cannot extend from multiple ancestors"
  });
  ...
  function isExtensible(obj) {
    return obj.isClass || typeof obj == "function";
  }
  ...
  function extend(base, ext) {  
    ...
    if(isExtensible(base) && isExtensible(ext)) {//this is a simple extending action
      inherits(base, ext);
      return base;
    }
  ...
  function _object_extend() {
    ...
    for (i = 0, len = args.length; i < len; i++) {
      if(isExtensible(args[i])) {//only extend from mixin or simple hash properties!
        console.warn(Q.WARNINGS["multiple_ancestors"]);
        continue;
      }
      extend(Ctor, args[i]);
    }
    ...
    Q.mixin(Ctor, {
      isClass: true,
      create: C.create || _object_create,
      extend: C.extend || _object_extend,
      statics: C.statics || _object_statics
    });
  ...
  Atom.prototype = {//setup a clean prototype
    ...
    /**
      Set this property to true after destroyed and this object will not interfear with others
     */
    destroyed: false,
